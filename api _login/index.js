...
app.delete('/cadastro/:id/:token', (req, res) => {  
  const id = req.params.id;
  const token = req.params.token;
  console.log(id, token);
  const jhy = jwt.verify(token, process.env.SECRET, function(err, decoded) {
    if (err){
      console.log(err)
    } else {
      execSQLQuery(`DELETE FROM cadastro WHERE idcadastro =${id}`, res);
    }
  }
